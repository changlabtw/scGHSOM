...
# readlink can cause problems on OSX, therefore, as fallback...
## "%{INSTALL_PATH}" is replaced by the IzPack-Installer
BASE_DIR=${BASE_DIR:-"%{INSTALL_PATH}"}
# Check if replacement worked, if not use "." as BASE_DIR
[ "$BASE_DIR" == "%{*" ] && BASE_DIR="."
...
EOF
	...
	usedMemory=$(( ( totalMemory / 1024 ) - 256))
...
#echo "$JAVA" "$JAVA_OPTS" -cp "$CP" at.tuwien.ifs.somtoolbox.apps.SOMToolboxMain $MAIN_CLASS "$@"
#echo "${CP}"
